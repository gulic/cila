...
mucho tiempo utilizando un depuraror  para encontrar fallos que no son
...
Este es el  aspecto que presenta el ddd cuando  lo arrancamos, vamos a
ir por pasos mostrando lo que normalmente un programador como nosotros
vamos a necesitar de un depurador.
...
En ddd  esto es  muy sencillo de  hacer y hay  varias maneras,  una de
ellas puede  ser escribir  el nombre  de la variable  en el  cuadro de
...
circunstancia pasajera, vamos  a colocar un {\em  breackpoint} o punto
de ruptura.

Los  breackpoints son  lugares  donde el  depurador  hará que  nuestro
...
breakpoint en el  ddd es simplemente desplazarse hasta  la línea donde
...
expresión  se cumpla,  por ejemplo,  si ponemos  el breackpoint  en un
...
hemos de poner en  propiedades del breackpoint es {\tt i  = 5000} y de
esa manera  nos saltaremos  las 5000 primeras  iteraciones que  no nos
interesan.

\begin{figure}[hbtp]
\centering
\includegraphics[width=\textwidth]{imagenes/ddd_breackpoint.eps}
\caption{DDD 4}
\end{figure}
...
las encontramos en una ventana que tenmos flotando sobre el depurador,
con lo cual no es complicado utilizarlas, siempre las tenemos a mano y
son las siguientes:

\begin{description}

\item[Step:]{(paso) esta herramienta nos  sirve, como su propio nombre
...
\item[Next:]{(siguiente) muy  parecida a step, solo  que pulsando {\tt
...
\item[Until:]{(hasta)  esta herramienta  nos  sirve  para ejecutar  el
programa hasta  la posición actual  del cursor,  es muy parecida  a un
breakpoint temporal.}

\item[Cont:]{(continuar)  saltamos  hasta  el siguiente  breakpoint  o
...
% os invito a experimentar cosas nuevas también aquí.

\subsection{Visión de punteros}

Casi seguro  que estabas deseando que  existiera una cosa como  la que
vas a ver así que no te  asombres de la potencia de este depurador, es
uno  de los  pocos que  lo hacen  y  de ahí  viene su  buena fama.  La
capacidad  de  la que  hablamos  es  la  de  {\sf ver}  los  punteros.
...
esto ya no es excusa para entregar  una práctica a medio hacer, el ddd
viene en nuestra ayuda y  tenemos la {\sf herramienta definitiva} para
la depuración con punteros

% al flexo de nuestra mesa de trabajo  (os prometo que eso no le gusta
% a nuestro sistema operativo favorito).\\
