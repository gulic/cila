...
escritos en Pascal, puedes consultar la sección sobre GDB, en este 
mismo manual.


Otros argumentos interesantes del compilador de FreePascal:

Sin entrar demasiado en las interioridades de FreePascal, creo que estas
opciones pueden ayudarte a la hora de compilar una práctica, o de
estructurar mejor tus proyectos. Ahí van:

\begin{verbatim} -FUdirectorio \end{verbatim}: Esta opción nos permite 
guardar las ``units'', una vez compiladas, en el directorio especificado.
Por ejemplo:
\begin{verbatim}
...
utilizamos el parámetro \begin{verbatim} -Fudirectorio \end{verbatim} 
(nótese que en este caso la `u' es minúscula). 
