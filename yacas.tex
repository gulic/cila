...
También   existe  una   interfaz  gráfica   llamada  Porteus.   Si  la
tienes  instalada  deberías  poder  iniciarla  ejecutando  el  comando
\verb+proteusworksheet+. Esta  utilidad te  proporcioa un  entorno más
cómodo e integrado  en el que puedes probar código  en el intérprete e
...
función  \verb+Ejample();+ varias  veces. Por  supuesto no  tienes que
teclear el comando todas las veces, ya que Yacas cuenta con edición de
línea al estilo de GNU Emacs (utiliza GNU readline). Esto quiere decir
que todos  los comandos  que ejecutes  en Yacas  se almacenarán  en un
...
Cuando  definimos una  función en  Yacas, la  parte a  la derecha  del
operador de asignación \verb+:=+ no se  evalúa sino que se asigna como
expresión a la  función. Si quieres que Yacas evalúe  la parte derecha
de  la  definición de  una  función  puedes  forzarlo con  la  función
\verb+Eval()+. Esto es  útil cuando la evaluación de  la parte derecha
resulta computacionalemente costosa, por ejemplo  en el caso de series
de Taylor.  Si evaluamos una  expansión en  series de Taylor  antes de
asignarla a la función, la máquina no tendrá que calcular la expansión
cada vez que la función sea llamada.
