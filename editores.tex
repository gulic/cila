...
opciones de este editor, se puede utilizar la opción {\tt Ctrl-k h}.
...
frecuencia al principio, cuando cometas algún error grave como teclear
una palabra sin pasar al modo  {\em edición}. En el modo {\em comando}
cada  tecla  tiene su  función,  y  es  diferente  además si  está  en
...
% Carlos (faraox): revisa este ejemplo para que quede claro

y si  queremos sustituir solo  la palabra vi  que se encuentre  en una
sola línea por vim, podremos utilizar:
...
Ahora   vamos  a   introducir   unos  elementos   nuevos  para   hacer
reemplazos y  busquedas más  ``profesionales''. Uno  de ellos  son los
{\em cuantificadores}.  Si necesitamos  buscar en  un documento  grandes
grupos de números de 9 cifras por ejemplo nos resultaría muy trabajoso
escribir \verb./\d\d\d\d\d\d\d\d\d.. Los  cuantificadores son diversas
...
Para  buscar mayúsculas  o minúsculas  hay algo  mejor que  los rangos
\verb.[a-z]. y  \verb.[A-Z]., que  son las  {\em clases}. Las  clases se
...
\begin{tabular}{|c|l|}
\hline
{\tt \verb.[[:lower:]].} & cualquier letra minúscula (incluída {\tt ñ}) \\
{\tt \verb.[[:upper:]].} & cualquier letra mayúscula (incluída {\tt Ñ}) \\
...
Tambien podemos cambiar el  orden de las frases de todo  un texto o de
zonas  específicas,  utilizando los  {\em  agrupadores}.  Se trata  de
...
{\tt \verb.\n.} & Hace referencia al n-ésimo grupo, con n entre 1 y 9 \\
