...
\section{El escritorio KDE}

KDE es un gestor de ventanas basado  en el entorno X-Window que nos da
un entorno gráfico amigable para  realizar las operaciones más comunes
con  nuestro  ordenador. Cuenta  con  un  panel donde  tener  nuestras
aplicaciones  organizadas,  un  gestor  de archivos  y  navegador  web
(Konqueror), y  otros muchos programas  para hacer nuestro  trabajo (o
nuestros ratos de ocio con el ordenador) mucho más cómodos.

Los programas  se encuentran situados  en el menú  con una ``K''  y un
engranaje (primer  icono de la  izquierda del panel),  distribuidos en
varios submenús, y clasificados por tipos. Normalmente, estos submenús
tendrán títulos como ``Programas'', ``Multimedia'', etc\dots

\begin{figure}[hbtp]
\centering
\includegraphics[width=0.88\textwidth]{imagenes/pantallazo_kde_descrito.eps}
\caption{Aspecto del escritorio KDE}
\end{figure}

Una de las combinaciones de teclas que más resuelven en KDE es ``Alt+F2'',
que nos muestra el diálogo ``Ejecutar'', en el cual podemos poner cualquier
comando

Este mismo diálogo nos da también la posibilidad de ejecutar el nuevo 
programa, tanto como otro usuario, como desde un terminal (esto se puede hacer
haciendo clic en el botón "Opciones".

\begin{figure}[hbtp]
\centering
\includegraphics[width=0.88\textwidth]{imagenes/pantallazo_kde_ejecutar.eps}
\caption{El diálogo ejecutar tras pulsar el botón ``Opciones''}
\end{figure}
