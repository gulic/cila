...
Unido es \texttt{ftp.tex.ac.uk}. También  existen diversos espejos. En
caso de no encontrarse en uno de estos países, siempre es mejor elegir
...
2e@\LaTeXe}\LaTeXe. Aquí de lo que te presentamos es \LaTeXe.
...
el movimiento cansino de los ojos del lector y no para que rellenen, a
ser posible, las páginas con un aspecto estéticamente bueno.
...
Esto indica qué tipo de documento  es el que quieres crear. Tras esto,
...
puedes  emplear  la instrucción  \begin{code}  \verb|\usepackage{...}|
\end{code}
...
\item[\normalfont\texttt{openright, openany}] \quad Hace que los
  capítulos comiencen o bien sólo en páginas a la derecha, o bien
...
\pagebreak[2]  \paragraph{Paquetes}   \index{\LaTeX!paquete}  Mientras
escribes  el documento,  probablemente  te  encuentres en  situaciones
donde el  \LaTeX{} básico  no basta  para realizar  lo que  buscas. Si
deseas incluir \wi{gráficos},  \wi{texto en color} o  el código fuente
...
\guia{} (si existe) se puede encontrar más información sobre los paquetes
disponibles en la instalación a la que se tenga acceso. Una buena fuente
...
Tras leer este tema deberías conocer  los elementos básicos de los que
se  compone  un  documento  de \LaTeXe.  En  este  tema  completaremos
...
cualquier circunstancia, o sea, que no se puede dividir.
...
esto,  los ``puntos  suspensivos'' no  se pueden  introducir con  tres
...
los  tipos de  acentos  que se  pueden aplicar  a  la letra  \emph{o}.
...
punto  tras una  letra mayúscula,  entonces  esto no  se considera  el
fin  de una  oración  ya que  los puntos  tras  las letras  mayúsculas
normalmente se utilizan para abreviaturas.

Cualquier excepción  a estas  reglas debes indicarla.  Una \emph{barra
invertida}  \verb|\|  antes  de  un   espacio  en  blanco  produce  un
espacio   en   blanco  que   no   se   ensanchará.  Un   carácter   de
tilde~`\verb|~|'\  genera  un  espacio   que  no  se  puede  ensanchar
...
ser necesario  compilar el documento  una tercera vez. \LaTeX{}  te lo
...
hayan  en  otros  lugares  del  documento.  \LaTeX{}  proporciona  las
...
\noindent  donde  \emph{marcador}  es   un  identificador  que  puedes
utilizar en  cualquier momento  para hacer  referencia al  elemento al
que  lo asocies.  \LaTeX{}  reemplaza \verb|\ref|  por  el número  del
apartado, subapartado,  figura, tabla o teorema  donde se introdujiste
la instrucción \verb|\label| correspondiente. La orden \verb|\pageref|
...
\verb|\end{verbatim}| aparecerá tal como se ha introducido, como si se
hubiese escrito con una máquina de escribir, con todos los espacios en
blanco y cambios de línea y sin interpretación de las instrucciones de
\LaTeX.
...
horizontales y verticales según desees. \LaTeX{} determina el ancho de
las columnas de modo automático.
...
\noindent define  el diseño del estadillo.  Puedes utilizar \texttt{l}
...
\LaTeX{}  te ofrece  dos entornos  para los  \wi{elementos flotantes}.
...
fuente de  frustaciones porque \LaTeX{}  nunca los pone  donde quieres
que vayan.
...
Cualquier  cosa   que  se   incluya  en   un  entorno   \ei{figure}  o
\ei{table} se tratará como  materia flotante. Ambos entornos flotantes
...
hayan en  las colas.  Si no  es posible, entonces  se trata  el primer
objeto  que  se  encuentra  en  las  colas  como  si  lo  acabases  de
...
corta  entre corchetes  tras  la  orden \verb|\caption|.  \begin{code}
\verb|\caption[Corto]{LLLLLaaaaaaaaarrrrrrrrgggggooooooo}| \end{code}
...
el  \emph{nombre}   de  la   instrucción  que   quieres  crear   y  la
...
En  determinados   casos  podrías   querer  utilizar   la  instrucción
...
carácter siguiente. Si deseas que una instrucción influya sobre varios
caracteres, entonces debes agruparlos empleando llaves (\verb|{...}|).
...
itálica. Para éstas \LaTeX{} proporciona las siguientes instrucciones:
\index{\LaTeX!funciones!matemáticas}
...
modo deseado si has utilizado una instrucción de cambio del tamaño del
tipo, o si se ha especificado la opción \texttt{11pt} o \texttt{12pt}.
Los paquetes  \pai{exscale} o  \pai{amstex} sirven para  corregir esta
anomalía.}.
...
matemático} Si no estás satisfecho con los espaciados que \TeX{} elige
dentro  de una  fórmula,  éstos se  pueden  alterar con  instrucciones
...
introducido a través  de la orden \texttt{\bs rd},  que previamente se
ha definido con  \texttt{\bs newcommand\{\bs rd\}\{\bs mathrm\{d\}\}}.
...
dividen automáticamente. Eres tú quien  debe determinar en qué lugares
se  deben  fraccionar  y  cuánto  se debe  sangrar.  Los  dos  métodos
...
utilizas  la instrucción  \verb|\textrm|, el  mecanismo de  cambio del
...
para que se mantenga activo el mecanismo de cambio de tamaño. Pero ten
cuidado, ya que  \ci{mathrm} sólo funcionará bien  con cosas pequeñas.
Los  espacios  no  son  aún  activos  y  los  caracteres  con  acentos
no  funcionan\footnote{El  paquete  AMS-\LaTeX{}  hace  que  la  orden
\ci{textrm} funcione bien con el cambio de tamaños.}.
...
Si  necesitas  componer  a  menudo   segmentos  de  texto  como  éste,
ahora   es   el  momento   idóneo   para   practicar  la   instrucción
\verb|\newenvironment|. Empléala  para crear un  entorno especializado
para   describir  variables.\index{\LaTeX!descripción   de  variables}
Revisa la descripción al final del tema anterior.
...
\begin{intro}
  Si ya te sientes lo sucifientemente seguro de ti mismo, entonces
...
documento (apartados,  notas al  pie\ldots). En algunos  casos podrías
...
debería  aparecer antes  de lo  indicado.  Observa la  posición de  la
...
ten presente un pequeño consejo:
...
separaciones  entre renglones,  puedes  cambiar su  valor poniendo  la
orden
