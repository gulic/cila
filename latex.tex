...
PostScript encapsulado  en sus documentos  o cómo añadir un  índice de
materias en su documento.
...
puedes   recurrir  a   cualquiera  de   los  servidores   de  archivos
de   \texttt{CTAN}\@.  En   la  República   Federal  de   Alemania  es
...
\begin{intro}
En la primera parte de este apartado te presentamos una visión general
de la filosofía e historia de \textrm{\LaTeXe}. En la segunda parte se
incide  en las  estructuras básicas  de un  documento de  \LaTeX. Tras
leer  este tema,  podrás  tener  un conocimiento  básico  del modo  de
funcionamiento de \LaTeX. Cuando  continúes leyendo, la información de
este apartado te ayudará a  integrar toda la información adicional que
puedas obtener sobre  \LaTeX, tanto en el resto de  los apartados como
de otros sitios.
\end{intro}
...
\index{\LaTeX!Lamport,  Leslie}Leslie   Lamport~\cite{llmanual}.  Utiliza  el
cajista \TeX{} como elemento de composición.

Desde diciembre de  1994, el paquete \LaTeX{}  está siendo actualizado
por  un equipo  llamado  \index{\LaTeX!LaTeX3@\LaTeX 3}\LaTeX  3, que  dirige
\index{\LaTeX!Mittelbach, Frank}Frank Mittelbach, para incluir algunas de las
...
primera versión \index{\LaTeX!LaTeX 2.09@\LaTeX{}  2.09}\LaTeX{} 2.09 hace ya
algunos años. Para distinguirla de la  vieja, a la nueva versión se le
llama \index{\LaTeX!LaTeX  2e@\LaTeXe}\LaTeXe. Aquí de lo  que te presentamos
es \LaTeXe.
...
finalmente  se va  a obtener  tras procesar  los ficheros  con \LaTeX.
Con  ellas puedes  siempre realizar  correcciones antes  de enviar  el
...
el movimiento fatigoso de los ojos  del lector y no para que rellenen,
a ser posible, las páginas con un aspecto estéticamente bueno.
...
\wi{espacio}  propiamente  dichos.   \emph{Varios}  espacios  seguidos
se  tratan  como  \emph{un}   \wi{espacio  en  blanco}.  Generalmente,
...
pueden imprimir  en fórmulas  matemáticas o  como acentos  con órdenes
específicas.

\paragraph{Las órdenes de \LaTeX{}}

En las  órdenes\index{\LaTeX!ordenes@órdenes} de  \LaTeX{} se  distinguen las
letras  mayúsculas y  las minúsculas.  Toman uno  de los  dos formatos
siguientes:

\begin{itemize}

\item Comienzan con  una \emph{barra invertida}\index{\LaTeX!barra invertida}
\verb|\|\cih{\bs} y  tienen un nombre  compuesto sólo por  letras. Los
nombres de  las órdenes acaban  con uno o  más espacios en  blanco, un
carácter especial o una cifra.

\item Se compone de una \emph{barra invertida} y un carácter especial.

\end{itemize}

\LaTeX{}  ignora los  espacios en  blanco  que van  tras las  órdenes.
Si  deseas introducir  \index{\LaTeX!espacio en  blanco!tras instrucción}  un
...
\item [\normalfont\texttt{article}] para artículos de revistas
  ...
  otros.\index{\LaTeX!articulo@artículo}%
...
\item [\normalfont\texttt{report}] para informes mayores que constan
  ...
  similares.\index{\LaTeX!informe}\index{\LaTeX!clase \texttt{report}@clase report}
\item [\normalfont\texttt{book}] para libros de
  verdad\index{\LaTeX!libro}\index{\LaTeX!clase \texttt{book}@clase book}
\item [\normalfont\texttt{slide}] para transparencias. Esta clase emplea
  tipos grandes \textsf{sans serif}.
  \index{\LaTeX!transparencias}\index{\LaTeX!clase \texttt{slide}@clase slide}
\end{description}
\end{lined}
\end{table}
...
\item[\normalfont\texttt{10pt}, \texttt{11pt}, \texttt{12pt}] \quad
  ...
  opción, se toma \texttt{10pt}.\index{\LaTeX!tamaño de los tipos!del
    documento}
\item[\normalfont\texttt{a4paper}, \texttt{letterpaper}, \ldots] \quad
  ...
  \texttt{legalpaper}. \index{\LaTeX!papel legal} \index{\LaTeX!tamaño del
    papel}\index{\LaTeX!papel DIN-A4} \index{\LaTeX!papel de carta} \index{\LaTeX!papel DIN-A5}\
  \index{\LaTeX!papel DIN-B5} \index{\LaTeX!papel ejecutivo}
...
\item[\normalfont\texttt{titlepage}, \texttt{notitlepage}] \quad
  ...
  y \texttt{book} sí.\index{\LaTeX!titlepage@\texttt{titlepage}}
...
\pagebreak[2]
\paragraph{Paquetes}
\index{\LaTeX!paquete} Mientras escribes el documento, probablemente te
...
\index{\LaTeX!""@\texttt{""}} que se usa en las máquinas de escribir. Para
...
especiales. En el idioma español\index{\LaTeX!espanol@español}, por ejemplo,
...
\index{\LaTeX!~@\verb.~.} \index{\LaTeX!tilde@tilde (\verb.~.)} \index{\LaTeX!.!  espacio
  tras}
...
índice\index{\LaTeX!indice@índice} correcto. En algunos casos puede ser
...
bordes).%
\index{\LaTeX!justificado a la izquierda}\index{\LaTeX!justificado a la derecha} El
...
\index{\LaTeX!alineación decimal}%
...
\begin{intro}
  !`Ahora a admirarse con este apartado! Vamos a
...
entre \ci{(} y \ci{)}, \index{\LaTeX!\$@\texttt{\$}} entre \texttt{\$} y
...
emplean: aquí sería más \emph{convencional} emplear `\emph{blackboard
  bold}' \index{\LaTeX!blackboard bold@\emph{blackboad bold}} \index{\LaTeX!simbolos
...
\ci{widehat}. Con el símbolo \verb|'|\index{\LaTeX!'@\verb"|'"|} se introduce
...
\textbf{fracción}\index{\LaTeX!fraccion@fracción} se pone con la orden
\ci{frac}\verb|{...}{...}|. Para los quebrados sencillos a veces
suele ser preferible utilizar el operador \verb|/|, %\index{\LaTeX!/@\verb|/|}
como en $1/2$.
\begin{example}
$1\frac{1}{2}$~horas
\begin{displaymath}
...
correcto del delimitador matemático\index{\LaTeX!delimitador!matemático}.
...
\index{\LaTeX!espaciado en modo matematico@espaciado en modo matemático}
...
\index{\LaTeX!ecuaciones largas} Las \textbf{ecuaciones largas} no se dividen
...
\index{\LaTeX!tamano del tipo@tamaño del tipo!para ecuaciones} En el modo
...
para describir variables.\index{\LaTeX!descripción de variables} Revisa la
...
\index{\LaTeX!tipo}\index{\LaTeX!tamaño del tipo} \LaTeX{} elige el tipo y el tamaño
...
\index{\LaTeX!separaciones entre renglones} Si quieres emplear mayores
...
teclearemos en el mismo directorio en el que está el fichero
...
Ahora podemos ver el resultado  con cualquier visualizador de ficheros
